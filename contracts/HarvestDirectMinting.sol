...
contract HarvestDirectMinting is TokenSwapPathRegistry {
    ...
    function harvest() external returns (uint256 cvxCrvHarvested) {
        ...
        uint256 threeCrvBalance = threeCrvToken.balanceOf(address(this));
         ...
         if (threeCrvBalance > 0) {
            ...
            if (usdcBalance > 0) {
                _swapExactTokensForTokens(sushiswap, usdc, usdcBalance, getTokenSwapPath(usdc, crv), 3);
            }
        }
        ...
        if (cvxTokenBalance > 0) {
            _swapExactTokensForTokens(sushiswap, cvx, cvxTokenBalance, getTokenSwapPath(cvx, crv), 3);
        }

        uint256 crvBalance = crvToken.balanceOf(address(this));

        crvDepositor.deposit(crvBalance, false);
